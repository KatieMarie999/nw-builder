...
//=============================================================================
// biud436 - RS_MirrorArea (Mod by DvLyon)
// RS_MirrorArea.js
//=============================================================================
...
/*:
 * @plugindesc This plugin allows you to indicate the mirror image the character on the screen <RS_MirrorArea> (Mod by DvLyon)
 * @author biud436
 *
 * @param Mirrors
 ...
 */
/*:ko
 * @plugindesc 캐릭터가 비치는 완벽한 거울 오브젝트를 만들 수 있습니다. <RS_MirrorArea> (Mod by DvLyon)
 * @author 러닝은빛(biud436)
 *
 * @param Mirrors
 * @text 거울 오브젝트 설정
 * @desc Set the required mask area for the mirror object. It can be a little complicated because there is no editor.
 * @default ["{\"note\":\"MIRROR_NORMAL\",\"w\":\"28\",\"h\":\"42\",\"mask_ox\":\"10\",\"mask_oy\":\"25\",\"char_ox\":\"10\",\"char_oy\":\"25\"}","{\"note\":\"MIRROR_DRESSER\", \"w\": \"34\",\"h\":\"15\", \"mask_ox\": \"10\", \"mask_oy\": \"-5\", \"char_ox\": \"10\", \"char_oy\": \"70\"}"]
 * @type struct<MirrorBase>[]
 *
 * @help
...
(function ($) {
  ...
  /**
   * Bitmap.snap Hide the character sprite on call.
   */
  ...
  /**
   * The disadvantage is that if you use a lot of masks, performance may decrease.
   * @param {Number} x 
   * @param {Number} y 
   * @param {Number} w 
   * @param {Number} h 
  ...
  Spriteset_Map.prototype.addChildMirrorImage = function (target, graphics, event, offset) {
    if (!this._mirrorCharacters) return false; // Without an object pool
    if (!this._tilemap) return false; // Without tile map

    // Create a left and right flipped character sprite.
    var child = new Sprite_Mirror(target);
    child.setProperties(graphics, event, offset);
    // Add them to the character object pool and tile map.
  ...
  Spriteset_Map.prototype.createMirrorImage = function (event, type, id) {
    ...
    // If you're a game player, consider all followers.
  ...
  Spriteset_Map.prototype.findAllTypeMirrors = function () {
    ...
    $gameMap.getRealEvents().forEach(function (event) {
      ...
      if (event === null || event === undefined) return false; //Is the event defined
      if (event._erased) return false; // Delete event temporarily
      if (event.findProperPageIndex() < 0) return false; // Event time conditions
      if (!event.page()) return false; // Page presence
      var eventlist = event.list();
      if (!eventlist) return false; // Event list exists

      // Search the entire event list.
