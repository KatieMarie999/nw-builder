...
//-----------------------------------------------------------------------------
/*:
 * @plugindesc (v.1.6) Fishing mini game that runs in it's own scene. For advanced users.
 * 
...
Scene_Fishing.prototype.createPond = function () {
  ...
  switch ($gameSystem.fishing.player.dir) {
    case 4: // left
      ...
      obj.x = x;
      obj.y = y;
      obj.width = $gameSystem.fishing.player.x - x - 100;
...
//Manage Fog
Spriteset_Fishing.prototype.validFog = function () {
  return false;
}
...
Game_Fish.prototype.doMove = function () {
  // horz
  if (this.dir > 0) {
    if (this.x < Galv.FISH.pond._endX) {
      this.x += this.dir;
    }
  }
  else if (this.dir < 0) {
    if (this.x > Galv.FISH.pond.x) {
      this.x += this.dir;
    }
  }
  // vert
  if (this.vDir > 0) {
    if (this.y < Galv.FISH.pond._floor) {
      this.y += this.vDir;
    }
  } else if (this.vDir < 0) {
    if (this.y > Galv.FISH.pond._surfaceY) {
      this.y += this.vDir;
    }
  }
};
