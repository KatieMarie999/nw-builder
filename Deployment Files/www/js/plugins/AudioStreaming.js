...
if (function() {
    'use strict';
    var parameters = PluginManager.parameters('AudioStreaming');
    var enabled = parameters.mode[0] === '1';
    var measured = parameters.mode[1] === '1';
...
AudioManager.audioFileExt = function() {
  ...
  {
    window.cordova = window.cordova || true;
  });
...
WebAudio.prototype._onDecode = function(result) {
    ...
    for (var w = 0; w < result.data.length; w++) {
        if (buffer.copyToChannel) {
            buffer.copyToChannel(result.data[w], w);
        } else {
            buffer.getChannelData(w).set(result.data[w]);
        }
    }
...
WebAudio.prototype._onLoad = function() {
    ...
    this._loadListeners.forEach(function(listener) {
      listener(); 
    });
...
Decrypter.decryptUint8Array = function(uint8Array) {
    ...
    for (var w = 0; w < this._headerlength; w++) {
        uint8Array[w] = uint8Array[w] ^ parseInt(this._encryptionKey[i], 16);
    }
    return uint8Array;
};
