...
DataManager.loadDataFile = function (name, src) 
{
  ...
  //  var url = 'data/' + src.replace("json", "glitch");
  //   xhr.open('GET', url);
  // }
  // else {
  var url = 'data/' + src;
  ...
  /*
    if (window.isProduction != null && window.isProduction == true) {
      ...
      xhr.onload = function () {
        if (xhr.status < 400) {
          ...
          var msg = BSON.deserialize(buffer);
          window[name] = msg;
...
DataManager.loadMapData = function (mapId) {
  ...
  if (mapId > 0) {
    ...
    //if (window.isProduction != null && window.isProduction == true) {
    //  fileName = 'Map%1.glitch'.format(mapId.padZero(3));
    //}
...
ImageManager.loadBitmap = function (folder, filename, hue, smooth) 
{
  if (filename) 
    ...
    //if(window.isProduction)
   // {
   //   path = folder + encodeURIComponent(filename) + '.img';
   // }
  //  else
  //  {
...
function createBlob(contents, contentType)
{
...
ImageManager.loadNormalBitmap = function (path, hue) 
{
  ...
  { 
   ...
   // { 
    ...
    // var contents = fs.readFileSync(decodeURIComponent(path),{encoding:'utf8'});
     //var dataUrl = "data: image/png;base64," + contents; 
     // var blob = createBlob(contents,contentType);
   //var blobUrl = URL.createObjectURL(blob);  
     //bitmap = Bitmap.load(dataUrl); 
   //  bitmap = Bitmap.load(blobUrl); 
...
ImageManager.requestBitmap = function (folder, filename, hue, smooth) {
  if (filename) 
  {
    ...
    if(window.isProduction)
    {
      var path = folder + encodeURIComponent(filename) + '.img';
...
ImageManager.requestNormalBitmap = function (path, hue) {
  ...
  if (!bitmap)
  {
   ...
   // {
    //  var contents = fs.readFileSync(decodeURIComponent(path),{encoding:'utf8'});
     // var imageUrl = "data:image/png;base64," + contents; 
     // var blob = createBlob(contents,contentType);
     // var blobUrl = URL.createObjectURL(blob);  
      //bitmap = Bitmap.request(path);
    //  bitmap = Bitmap.request(blobUrl); 
   // bitmap.addLoadListener(function ()
    //   {
     //    bitmap.rotateHue(hue);
         //URL.revokeObjectURL(blobUrl);
...
AudioManager.playBgm = function (bgm, pos) {
 ...
 //   var fs = require('fs');
   // const contentType = 'audio/ogg';
    //var zlib = require('zlib'); 
    var extension = path.ext(bgm.name);

    //var compress = zlib.createGzip(); 
    //TODO: replace bgm play with PIXI sound 
 //   var fileName = bgm.name.replace(extension, ".heh");
  
  //  var contents = fs.readFileSync(decodeURIComponent(fileName), { encoding:'utf8'});
      
 // var blob = createBlob(contents,contentType);
 //  var blobUrl = URL.createObjectURL(blob);  

 
  //  snd = PIXI.sound.Sound.from(blobUrl);
  //  snd.loop = true;
  ///  snd.play();
 // }
 // else {
...
AudioManager.audioFileExt = function () {
  ...
  /*
  else if(WebAudio.canPlayOgg() && !Utils.isMobileDevice() && window.isProduction)
  {
    return '.heh'; 
  }*/ 
...
AudioManager.shouldUseHtml5Audio = function () {
  ...
  // Atsuma-ru asked to force webaudio there too, so just return false for ALL    // return Utils.isAndroidChrome() && !  er.hasEncryptedAudio;
  return false;
};
...
SceneManager.initGraphics = function ()
{
  ...
  //if(window.isProduction)
 // {
   // var base64Data = readBase64Data('img/system/Loading.img'); 
    //Graphics.setLoadingImage("data:image/png;base64," + base64Data); 
   // var blob = createBlob(base64Data,"image/png");
 //  var blobUrl = URL.createObjectURL(blob);  
   // Graphics.setLoadingImage("img/system/Loading.img");
  //  Graphics.setLoadingImage(blobUrl);
//  }
 // else
 // {
...
function readBase64Data(path)
{
  var img = ''; 
  var buffer = []; 
   var fs = require('fs');
  img = fs.readFileSync(path, { encoding:'utf8'}); 
      return img; 
}
  ...
  plugins.forEach(function (plugin) {
    if (plugin.status && !this._scripts.contains(plugin.name)) {
      ...
      if (!window.isProduction) {
        ...
        this.loadScript(plugin.name + '.glitch');
      }
...
PluginManager.loadScript = function (name) {
  ...
  if (!window.isProduction) {
    ...
    nw.Window.get().evalNWBin(null, "js/plugins/" + name);
  }
};
